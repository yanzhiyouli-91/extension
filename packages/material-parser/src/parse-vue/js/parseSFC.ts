...
export default async function parseSFC(
  ...
  const pathResolver = makePathResolver(
    path.dirname(opt.filePath),
    opt.alias,
    opt.modules,
  );

  // use padding so that errors are displayed at the correct line
  const { descriptor: parts } = cacher(
    () => parseComponent(source, { pad: 'line' }),
    source,
  );
  ...
  if (parts.template) {
    ...
    const addTemplateHandlers: TemplateHandler[] =
      opt.addTemplateHandlers || [];
  ...
  if (parts.customBlocks) {
    ...
    const docsBlocks = parts.customBlocks
      .filter(
        (block) =>
          block.type === 'docs' && block.content && block.content.length,
      )
  ...
  if (documentation && !documentation.get('displayName')) {
    ...
    documentation.set(
      'displayName',
      displayName.toLowerCase() === 'index' ? dirName : displayName,
    );
  }
...
async function parseScriptTag(
  ...
  if (extSrc && typeof extSrc === 'string') {
    ...
    if (extSource.length) {
      ...
      opt.lang =
        (scriptTag &&
          scriptTag.attrs &&
          /^tsx?$/.test(scriptTag.attrs.lang as string)) ||
        /\.tsx?$/i.test(extSrc)
          ? 'ts'
          : 'js';
  ...
  opt = isSetupScript
    ? { ...opt, scriptPreHandlers: [], scriptHandlers: setupHandlers }
    : opt;
