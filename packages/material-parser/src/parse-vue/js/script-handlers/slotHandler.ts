    ...
    visit(renderValuePath.node, {
      ...
      visitCallExpression(pathCall) {
        if (
          ...
          const doc = documentation.getSlotDescriptor(
            pathCall.node.callee.property.name,
          );
          const comment = getSlotComment(pathCall, doc);
          const bindings = pathCall.node.arguments[0];
          if (
            bindings?.type === 'ObjectExpression' &&
            bindings.properties.length
          ) {
...
function isStatement(path: NodePath): boolean {
  return (
    path &&
    (bt.isDeclaration(path.node) ||
      bt.isReturnStatement(path.node) ||
      bt.isIfStatement(path.node))
  );
}
...
export function parseSlotDocBlock(str: string, descriptor: SlotDescriptor) {
  ...
  if (slotTags.length) {
    ...
    if (
      description &&
      (!descriptor.description || !descriptor.description.length)
    ) {
    ...
    const tags = jsDoc.tags.filter(
      (t) => t.title !== 'slot' && t.title !== 'binding',
    );
    if (tags.length) {
      ...
      if (bt.isIdentifier(prop.key)) {
        ...
        const description: string | boolean | undefined =
          prop.leadingComments && prop.leadingComments.length
            ? parseDocblock(
                prop.leadingComments[prop.leadingComments.length - 1].value,
              )
            : undefined;
        if (!description) {
...
function getBindingsFromJSX(
  attr: bt.JSXAttribute,
  bindings: ParamTag[] | undefined,
): ParamTag {
  const name = attr.name.name as string;
  const descbinding = bindings
    ? bindings.filter((b) => b.name === name)[0]
    : undefined;
  if (descbinding) {
    return descbinding;
  }
