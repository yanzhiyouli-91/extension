  ...
  visit(path.node, {
    visitCallExpression(pathExpression) {
      if (
        ...
        const eventTags = doclets.tags
          ? doclets.tags.filter((d) => d.title === 'event')
          : [];
        ...
        if (
          evtDescriptor.properties &&
          evtDescriptor.properties.length < args.length - 2
        ) {
 ...
 * @param path
 */
export function eventHandlerEmits(
  documentation: Documentation,
  path: NodePath,
) {
  const emitsPath = path
    ...
    .filter(
      (p: NodePath) =>
        bt.isObjectProperty(p.node) && getMemberFilter('emits')(p),
  ...
  if (bt.isArrayExpression(emitsObject.node)) {
    ...
    emitsObject
      .get('properties')
      .value.forEach((event: bt.ObjectProperty, i: number) => {
...
export function eventHandlerMethods(
  ...
  const methodsPath = path
    ...
    .filter(
      (p: NodePath) =>
        bt.isObjectProperty(p.node) && getMemberFilter('methods')(p),
    ) as NodePath<bt.ObjectProperty>[];
  ...
  if (bt.isObjectExpression(methodsObject.node)) {
    methodsObject.get('properties').each((p: NodePath) => {
      ...
      const firesTags = jsDocTags.filter(
        (tag) => tag.title === 'fires',
      ) as Tag[];
      firesTags.forEach((tag) => {
        ...
        let commentIndex = 1;
        do {
          currentBlock = getCommentBlockAndTags(commentedMethod, {
          ...
          if (
            ...
            currentBlock.tags.some(
              (tag: any) => tag.title === 'event' && tag.content === eventName,
            )
          ) {
...
export function setEventDescriptor(
  ...
  eventDescriptor.type = typeTags.length
    ? { names: typeTags.map((t) => (t as TypedParamTag).type.name) }
    : eventDescriptor.type;

  const propertyTags = nonNullTags.filter((tg) =>
    PROPERTY_TAGS.includes(tg.title),
  );
  if (propertyTags.length) {
    eventDescriptor.properties = propertyTags.map((tg: any) => {
      return {
        type: { names: [tg.type.name] },
        name: tg.name,
  ...
  const tags = nonNullTags.filter(
    (tg: BlockTag) =>
      tg.title !== 'type' && tg.title !== 'property' && tg.title !== 'event',
  );
