...
export default async function classPropHandler(
  ...
  if (bt.isClassDeclaration(path.node)) {
    const config = getArgFromDecorator(
      path.get('decorators') as NodePath<bt.Decorator>,
    );
    ...
    path
      ...
      .get('body')
      .filter(
        (p: NodePath) => bt.isClassProperty(p.node) && !!p.node.decorators,
      )
      .forEach((propPath: NodePath<bt.ClassProperty>) => {
        const propDeco = (propPath.get('decorators') || []).filter(
          (p: NodePath<bt.Decorator>) => {
        ...
        const propName = bt.isIdentifier(propPath.node.key)
        ...
        if (propPath.node.typeAnnotation) {
          const values =
            ...
            getValuesFromTypeAnnotation(
              propPath.node.typeAnnotation.typeAnnotation,
            );
          if (values) {
