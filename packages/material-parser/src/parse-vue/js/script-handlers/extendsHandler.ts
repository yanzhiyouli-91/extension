...
const extendsHandler: ScriptHandler = async (
  ...
  const variablesResolvedToCurrentFile = resolveLocal(astPath, [
    extendsVariableName,
  ]);
...
function getExtendsVariableName(compDef: NodePath): string | undefined {
  ...
  if (extendsVariable) {
    ...
    return extendsValue && bt.isIdentifier(extendsValue)
      ? extendsValue.name
      : undefined;
  }
  return undefined;
...
function getExtendsVariableNameFromCompDef(
  compDef: NodePath,
): NodePath | undefined {
  if (!compDef) {
    return undefined;
  ...
  const pathExtends = compDefProperties.value
    ? compDefProperties.filter(
        (p: NodePath<bt.Property>) =>
          bt.isIdentifier(p.node.key) && p.node.key.name === 'extends',
      )
    : [];
