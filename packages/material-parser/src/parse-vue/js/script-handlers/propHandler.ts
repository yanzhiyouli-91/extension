  ...
  if (bt.isObjectExpression(propsValuePath.node)) {
    ...
    await Promise.all(
      objPropFiltered.map(async (prop) => {
        ...
        if (
          bt.isArrayExpression(propValuePath.node) ||
          bt.isIdentifier(propValuePath.node)
        ) {
          ...
          const finalPropValuePathExpression =
            ...
            bt.isTSUnknownKeyword(
              propValuePathExpression.get('typeAnnotation').node,
            )
...
export function describeType(
  ...
  if (typeArray.length) {
    return describeTypeAndValuesFromPath(
      typeArray[0].get('value'),
      propDescriptor,
    );
  } else {
    ...
    if (defaultArray.length) {
      ...
      if (bt.isObjectProperty(typeNode)) {
        const func =
          bt.isArrowFunctionExpression(typeNode.value) ||
          bt.isFunctionExpression(typeNode.value);
        const typeValueNode = defaultArray[0].get('value')
          .node as ValueLitteral;
...
export function getTypeFromTypePath(
  ...
  const typeName = !typeNode
    ...
    ? print(typeNode.value.expression).code +
      (typeNode.value.typeParameters
        ? print(typeNode.value.typeParameters).code
        : '')
    : print(typeNode).code;
  return {
 ...
 */
function getValuesFromTypePath(
  typeAnnotation: bt.TSType,
): string[] | undefined {
...
export function getValuesFromTypeAnnotation(
  type: bt.TSType,
): string[] | undefined {
  if (
    bt.isTSUnionType(type) &&
    type.types.every((t) => bt.isTSLiteralType(t))
  ) {
...
export function describeRequired(
  ...
  const requiredNode = requiredArray.length
    ? requiredArray[0].get('value').node
    : undefined;
  const required =
    requiredNode && bt.isBooleanLiteral(requiredNode)
      ? requiredNode.value
      : undefined;
  ...
  if (defaultArray.length) {
    ...
    if (propType === 'object' || propType === 'array') {
      if (defaultValueIsProp) {
        ...
        const isArrowFunction = bt.isArrowFunctionExpression(
          defaultFunction.node,
        );
        const isOldSchoolFunction = bt.isFunctionExpression(
          defaultFunction.node,
        );
        ...
        if (
          ...
          (bt.isIdentifier(defaultFunction.node) &&
            defaultFunction.node.name === 'undefined')
        ) {
        ...
        if (isArrowFunction) {
          ...
          if (bt.isBlockStatement(arrowFunctionBody.node)) {
            const rawValueParsed =
              getRawValueParsedFromFunctionsBlockStatementNode(
      ...
      const defaultBlockStatementNode: bt.BlockStatement =
        defaultBlockStatement.node;
...
async function describeValues(
  ...
  const validatorArray = propPropertiesPath.filter(
    getMemberFilter('validator'),
  );
...
export function extractValuesFromTags(propDescriptor: PropDescriptor) {
  if (propDescriptor.tags && propDescriptor.tags.values) {
    const values = propDescriptor.tags.values.map((tag) => {
      ...
      const choices =
        typeof description === 'string' ? description.split(',') : undefined;
      if (choices) {
...
function getModelPropName(path: NodePath): string | null {
  ...
  const modelPropertyNamePath = modelValue
    ...
    .filter(
      (p: NodePath) =>
        bt.isObjectProperty(p.node) && getMemberFilter('prop')(p),
    );
