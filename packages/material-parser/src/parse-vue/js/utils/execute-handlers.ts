...
async function executeHandlers(
  ...
  const compDefs = componentDefinitions
    .keys()
    .filter(
      (name) => name && (!opt.nameFilter || opt.nameFilter.indexOf(name) > -1),
    );
  ...
  const docs = await Promise.all(
    compDefs.map(async (name) => {
      ...
      const doc =
        (compDefs.length > 1 && name !== 'default'
          ? undefined
          : documentation) || new Documentation(opt.filePath);
  ...
  return docs.sort((a, b) =>
    a.get('exportName') === 'default'
      ? -1
      : b.get('exportName') === 'default'
      ? 1
      : 0,
  );
}
