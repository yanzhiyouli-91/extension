...
function getParamInfo(content: string, hasName: boolean) {
  content = content || '';
  const typeSlice: string = matchRecursiveRegExp(content, '{', '}')[0] || '';
  const param: Param =
    hasName || typeSlice.length
      ? { type: getTypeObjectFromTypeString(typeSlice) }
      : {};
...
export default function getDocblockTags(str: string): DocBlockTags {
  ...
  lines.forEach((line) => {
    ...
    if (TYPED_TAG_TITLES.includes(title)) {
      tags.push({
        title,
        ...getParamInfo(tagContents, !UNNAMED_TAG_TITLES.includes(title)),
      });
  ...
  const description = accNonTagLines.trim().length
    ? accNonTagLines.trim()
    : undefined;
