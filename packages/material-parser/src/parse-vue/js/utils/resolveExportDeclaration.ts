...
export default function resolveExportDeclaration(
  ...
  if (bt.isExportDefaultDeclaration(path.node)) {
    ...
    if (declaration && bt.isVariableDeclaration(declaration.node)) {
      declaration
        .get('declarations')
        .each((declarator: NodePath<bt.VariableDeclarator>) => {
...
function getDefinitionsFromPathSpecifiers(
  path: NodePath,
  defs: Map<string, NodePath>,
) {
    ...
    (specifier: NodePath<bt.ExportSpecifier | bt.ExportNamespaceSpecifier>) => {
      if (bt.isIdentifier(specifier.node.exported)) {
        defs.set(
          ...
          bt.isExportSpecifier(specifier.node)
            ? specifier.get('local')
            : specifier.get('exported'),
        );
      }
    },
  );
