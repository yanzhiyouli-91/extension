...
const getTypeDefinitionFromIdentifierFromModule = (
  ...
  pathResolver: (
    filePath: string,
    originalDirNameOverride?: string,
  ) => string | null,
) => {
  ...
  visit(astPath.program, {
    ...
    visitTSInterfaceDeclaration(nodePath) {
      if (
        ...
        nodePath.node.id.name === typeName
      ) {
        ...
        if (nodePath.value.extends) {
          const parentInterfaces = nodePath.value
            .extends as TSExpressionWithTypeArgumentsKind[];
          parentInterfaces.forEach((parentInterface) => {
            ...
            parentInterfaceBody?.value.forEach(
              (parentInterfaceProp: TSPropertySignatureKind) => {
                if (
