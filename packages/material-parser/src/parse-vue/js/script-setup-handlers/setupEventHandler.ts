  ...
  visit(astPath.program, {
    visitCallExpression(nodePath) {
      if (
        ...
        if (
          bt.isTSTypeParameterInstantiation(nodePath.get('typeParameters').node)
        ) {
          nodePath.get('typeParameters', 'params').each((param: NodePath) => {
            if (bt.isTSTypeLiteral(param.node)) {
              ...
              bt.isTSTypeReference(param.node) &&
              bt.isIdentifier(param.node.typeName)
            ) {
