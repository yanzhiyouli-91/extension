...
import getTypeFromAnnotation, {
  decorateItem,
} from '../utils/getTypeFromAnnotation';
  ...
  visit(astPath.program, {
    visitCallExpression(nodePath) {
      const hasDefaults =
        ...
        nodePath.node.callee.name === 'withDefaults';
      const normalizedNodePath = hasDefaults
      ...
      if (
        ...
        if ((normalizedNodePath.node as any).typeParameters) {
          const typeParamsPath = normalizedNodePath.get(
            'typeParameters',
            'params',
            0,
          );
          if (bt.isTSTypeLiteral(typeParamsPath.node)) {
            getPropsFromLiteralType(
              documentation,
              typeParamsPath.get('members'),
            );
