...
export default function slotHandler(
  ...
  if (isBaseElementNode(templateAst) && templateAst.tag === 'slot') {
    const nameProp = templateAst.props
      .filter(isAttributeNode)
      .find((b) => b.name === 'name');
    let slotName =
      nameProp && nameProp.value ? nameProp.value.content : undefined;
    ...
    const simpleVBind = bindings.find(
      (b) => isDirectiveNode(b) && !b.arg,
    ) as DirectiveNode;
    let rawVBind = false;
        ...
        visitObjectExpression(path) {
          path.get('properties').each((property: NodePath) => {
            ...
            if (bt.isProperty(node) || bt.isObjectProperty(node)) {
              ...
              const bindingDesc = bindingDescriptors.filter(
                (t) => t.name === name,
              )[0];
              simpleBindings.push(
                bindingDesc
    ...
    const restOfBindings = bindingDescriptors.filter(
      (bindFromComment) =>
        !slotDescriptor.bindings?.some(
          (bindFromData) => bindFromData.title === bindFromComment.title,
        ),
    );
